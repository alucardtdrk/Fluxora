...
export function readSession(req: any) { return verifySessionToken(parse(req.headers.cookie || "")[COOKIE_NAME]); }
export function setSessionCookie(res: any, user: AuthenticatedUser) {
...
export function clearSessionCookie(res: any) {
  if (typeof res.clearCookie === "function") {
    res.clearCookie(COOKIE_NAME, { httpOnly: true, secure: true, sameSite: "none", path: "/", maxAge: -1 });
    return;
  }
...
export function createOAuthState(res: any) {
...
export function consumeOAuthState(req: any, res: any, receivedState?: string) {
