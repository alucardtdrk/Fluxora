...
export async function listArchivedExecutions(period: ArchivePeriod) {
  ...
  const items = rows
    ...
    .filter((row) => row.id && row.id !== "teste_inicial");
...
export async function getArchiveDiagnostics() {
  ...
  const [state, totalArchived] = await Promise.all([getArchiveSyncState(), countFirestoreCollection(logsCollection())]);
  return { configured: true, totalArchived, state };
}
