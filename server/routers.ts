...
export const appRouter = router({
  ...
  admin: router({
    ...
    syncGoogleWorkspace: adminProcedure.input(z.object({ startIndex: z.number().int().min(0).max(14) })).mutation(({ input, ctx }) => audited({ action: "google_workspace.sync", category: "configuration", actor: ctx.user.email, actorRole: ctx.user.role, targetType: "google_workspace", summary: "Executou um lote da sincronização de segurança do Google Workspace" }, () => syncGoogleWorkspaceSecurity(input.startIndex))),
